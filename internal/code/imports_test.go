...
func TestImportPathForDir(t *testing.T) {
	...
	assert.Equal(t, "github.com/ervitis/gqlgen/internal/code", ImportPathForDir(wd))
	assert.Equal(t, "github.com/ervitis/gqlgen/api", ImportPathForDir(filepath.Join(wd, "..", "..", "api")))

	// doesnt contain go code, but should still give a valid import path
	assert.Equal(t, "github.com/ervitis/gqlgen/docs", ImportPathForDir(filepath.Join(wd, "..", "..", "docs")))

	// directory does not exist
	assert.Equal(t, "github.com/ervitis/gqlgen/dos", ImportPathForDir(filepath.Join(wd, "..", "..", "dos")))
